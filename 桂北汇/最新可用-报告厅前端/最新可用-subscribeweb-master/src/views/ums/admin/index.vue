      ...
      <el-table ref="adminTable" :data="list" style="width: 100%" v-loading="listLoading" border>
        ...
        <el-table-column label="部门" align="center">
          <template slot-scope="scope">{{
            borrowUnitMap.get(scope.row.departmentId)
          }}</template>
        </el-table-column>
    ...
    <el-dialog :title="isEdit ? '编辑用户' : '添加用户'" :visible.sync="dialogVisible" width="40%">
      <el-form :model="admin" ref="adminForm" label-width="150px" size="small">
        ...
        <el-form-item label="部门：">
          <!--          <el-input v-model="admin.departmentId" style="width: 250px"></el-input>-->
          <el-select v-model="admin.departmentId" placeholder="请选择">
  ...
  export default {
    ...
    methods: {
      ...
      handleUpdate(index, row) {
        ...
        this.options = [];
        for (let i = 1; i <= this.borrowUnitMap.size; i++) {
          this.options.push({
            value: i,
            label: this.borrowUnitMap.get(i)
          });
        }
        this.value = this.borrowUnitMap.get(row.departmentId);
      },
      handleDialogConfirm() {
